...
public class ScoringFunction {
	...
	private static final int TOTALDOC = 2704533;
	...
	*/
	double getScore(String query, long docID, int rawDf) {
		...
		for (String term : terms) {
			if (term.matches("\\d+\\(\\w+\\s\\w+\\)")) {  // for proximity operator
				...
				score += dealWithNormalTerms(term, docID, rawDf);
			}
		}
	...
	*/
	/*private double dealWithProximityOperation(String term, long docID) {
	...
	*/
	private double dealWithNormalTerms(String term, long docID, int rawDf) {
		// filtering
		...
		return getTfIdf(term, docID, rawDf);
	}
	...
	*/
	private FreqAndLists getPositionalInvertedIndexForProximity(String phrase1, String phrase2, int proximityWindow) {  // generate a FreqAndLists structure for the combine phrases
		...
		while (i < postingList1.size() && j < postingList2.size()) {
			...
			long docID1 = list1.getDocID();
			long docID2 = list2.getDocID();
			if (docID1 == docID2) {
	...
	*/
	private double getTfIdf(String term, long docID, int rawDf) {
		double idf = getIdf(term, rawDf);
		return getTf(term, docID) * idf;
		//return getTf(term, docID);
	}
	 ...
	 *  @return the double tf-wt
	*/
	/*private double getTfIdf(FreqAndLists fl, long docID) {  // for proximity operator
		double idf = getIdf(fl);
		return getTf(fl, docID) * idf;
		//return getTf(fl, docID);
	}*/
	 ...
	 *  @return idf
	*/
	private double getIdf(String term, int rawDf) {
		//int rawDf = getRawDf(term);
		//System.out.println(term + " " + rawDf);
		return Math.log10(TOTALDOC * 1.0 / rawDf);
	}
	...
	*/
	/*private double getIdf(FreqAndLists fl) {  // for proximity operator
		...
		return Math.log10(numberOfDocument * 1.0 / rawDf);
	}*/
	 ...
	 *  @return log tf
	*/
	private double getTf(String term, long docID) {
	 ...
	 *  @return log tf
	*/
	private double getTf(FreqAndLists fl, long docID) {  // for proximity operator
	...
	*/
	//private int getRawDf(String phrase) {
		...
		//return GetRawDf.traverseDfs(DFDIR, phrase);
	//}
	...
	*/
	 ...
	 *  @return raw tf
	*/
	private int getRawTf(String phrase, long docID) {
		FreqAndLists value = map.get(phrase);
	 ...
	 *  @return raw tf
	*/
	private int getRawTf(FreqAndLists fl, long docID) {  // for proximity operator
